...
export default function ReunioesOportunidadeListBO(){
    ...
    function LoadFillData(){
        ...
        return sortedData.map((data)=>{
            const dataReuniao = moment(data.data_reuniao).tz('Europe/London').format('DD/MM/YYYY HH:mm'); // Ajuste o fuso horário conforme necessário
            return(
                <tr key={data.id} > {/*onClick={()=> navigate('/backoffice/oportunidades/'+ data.id)}*/}
                    ...
                    <td className='align-middle'>{dataReuniao}</td>
                    <td className='align-middle'>
                        <Link className="btn btn-outline-info" onClick={(e) => e.stopPropagation()}  to={"/backoffice/oportunidades/"+oportunidadeId+"/reunioes/"+data.id+"/update"} >Edit</Link>
                    </td>
                </tr>
            )
        });
    ...
    return (
        <div className="tabela-centro">
            ...
            <Link className="btn_adicionar " to={"/backoffice/oportunidades/"+ oportunidadeId  +"/reunioes/create"}>Criar Nova</Link>
            {isLoading? (
