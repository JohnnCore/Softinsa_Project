...
export default function NotasInteracoesUpdate() {
    const navigate = useNavigate()

    const [isLoading, setIsLoading] = useState(true);

    const { oportunidadeId, contactoId, interacaoId, notaId } = useParams();
    ...
    useEffect(() => {
        const LoadNota = () => {
            const url = "https://pint-backend.onrender.com/notas/get/" + notaId + "/interacoes/" + interacaoId + "/contactos/" + contactoId + "/oportunidades/" + oportunidadeId
        ...
        LoadNota()
    }, [notaId, contactoId, interacaoId, oportunidadeId]);
    ...
    function SendUpdate() {
        if (campTitulo === "") {
            ...
            const baseUrl = "https://pint-backend.onrender.com/notas/update/" + notaId + "/interacoes/" + interacaoId + "/contactos/" + contactoId + "/oportunidades/" + oportunidadeId
            ...
            axios.post(baseUrl, datapost, { headers: authHeader() })
                .then(response => {
                    if (response.data.success === false) {
                        ...
                        navigate('/backoffice/oportunidades/' + oportunidadeId + '/contactos/' + contactoId + '/interacoes/' + interacaoId + "/notas")
                    }
                })
                .catch((error) => {
        ...
        <div className='tabela-centro'>
            <nav aria-label="breadcrumb">
                <ol className="breadcrumb">
                    ...
                    <li className="breadcrumb-item"><Link to={"/backoffice/oportunidades/" + oportunidadeId + "/contactos/" + contactoId + "/interacoes/" + interacaoId + "/notas"}>Notas</Link></li>
