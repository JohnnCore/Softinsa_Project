...
export default function NotasReuniaoIdeiaListBO(){
    ...
    useEffect(() => {
        LoadNotasReuniaoIdeiaBO();
    },[]);

    function LoadNotasReuniaoIdeiaBO() {
        const url = "https://pint-backend.onrender.com/notas/reunioes/"+reuniaoId+"/ideias/"+ideiaId+"/list"
            ...
            return(
                <tr key={data.id} > {/*onClick={()=> navigate('/backoffice/oportunidades/'+ data.id)}*/}
                    ...
                    <td className='align-middle'>
                        <Link className="btn btn-outline-info" onClick={(e) => e.stopPropagation()}  to={"/backoffice/ideias/"+ideiaId+"/reunioes/"+reuniaoId+"/notas/"+data.id+"/update"} >Edit</Link>
                    </td>
                    <td className='align-middle'>
    ...
    return (
        <div className="tabela-centro">
            ...
            <ul className="nav nav-pills nav-fill">
                ...
                <li className="nav-item">
                    <Link className="nav-link active" aria-current="page" to={"/backoffice/ideias/" + ideiaId + "/reunioes/"+reuniaoId+"/notas"}>Notas</Link>
                </li>
                <li className="nav-item">
                    <Link className="nav-link" aria-current="page" to={"/backoffice/ideias/" + ideiaId + "/reunioes/"+reuniaoId+"/ficheiros"}>Ficheiros</Link>
                </li>
            </ul>
            <Link className="btn_adicionar " to={"/backoffice/ideias/"+ideiaId+"/reunioes/"+reuniaoId+"/notas/create"}>Criar Nova</Link>
            {isLoading? (
